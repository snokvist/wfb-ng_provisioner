...
REBOOT_THRESHOLD=112        # Reboot threshold
...
while true; do
    ...
    if [ "$max_temp" -ge "$REBOOT_THRESHOLD" ]; then
        ...
        echo "Throttling VTX. Reboot imminent, return to home..."
    elif [ "$max_temp" -ge "$WARNING2_THRESHOLD" ]; then
        echo "Warning: High temperature detected. VTX will soon thermal throttle..."
