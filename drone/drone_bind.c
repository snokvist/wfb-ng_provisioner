...
#define EXIT_FLASH  4
#define EXIT_BACKUP 5  // <-- New exit code for BACKUP

// Global flag for debug output.
static int debug_enabled = 0;

// Global bandwidth limit in bits/second (default: 1 Mbit/s).
static double g_bw_limit_bits = 1000000.0;  // 1,000,000 bits/sec = 1 Mbit/s
...
// Read entire file into a dynamically allocated string (text mode).
...
// Base64 encode a given binary buffer (with standard padding).
...
/*
 * Write 'data_len' bytes from 'data' to 'fp' at up to 'bits_per_sec' (e.g. 1e6 bits/sec).
 * Uses a chunked approach + monotonic clock to avoid exceeding the rate.
 */
static void throttle_write(FILE *fp, const char *data, size_t data_len, double bits_per_sec) {
    ...
    while (total_sent < data_len) {
        ...
        if (n < send_now) {
            // If we failed to write the full chunk, likely the connection closed.
            // We'll just break out; the caller can handle partial write if needed.
            total_sent += n;
            break;
        }
        
        total_sent += n;
        fflush(fp);  // Ensure data is actually sent out in a timely manner.
        ...
        // How many bytes we are allowed to have sent up to now
        double allowed_bytes = bytes_per_sec * elapsed;

        // If we've sent more than allowed, sleep briefly
        if (total_sent > allowed_bytes) {
            double excess_bytes = total_sent - allowed_bytes;
            double wait_time = excess_bytes / bytes_per_sec; // seconds
            if (wait_time > 0) {
...
// INFO: gather various system info, remove newlines, base64-encode, and send (throttled).
int cmd_info(const char *arg, FILE *client_file, int force_listen) {
    ...
    if (!ipcinfo_out)     ipcinfo_out     = strdup("Failed to execute ipcinfo command");
    if (!lsusb_out)       lsusb_out       = strdup("Failed to execute lsusb command");
    if (!osrelease_out)   osrelease_out   = strdup("Failed to read /etc/os-release");
    ...
    if (encoded_response) {
        ...
        // Send "OK\t" immediately (small)
        fprintf(client_file, "OK\t");
        fflush(client_file);
        
        // Now throttle the main base64 data
...
/*--------------------------------------------------
 * BACKUP (new command) - with throttled output
 *--------------------------------------------------*/
int cmd_backup(const char *arg, FILE *client_file, int force_listen) {
    (void)arg;  // Not used; BACKUP has no base64 argument
    debug_print("Received BACKUP command\n");
    
    // 1. Run generate_backup.sh
    int ret = system("generate_backup.sh /etc/backup_these_files.txt");
    if (ret == -1) {
    ...
    // 6. If not force_listen, terminate with EXIT_BACKUP (5)
    if (!force_listen) {
        return EXIT_BACKUP;
    }
    return 0;
    ...
    for (int i = 1; i < argc; i++) {
        if (strcmp(argv[i], "--help") == 0) {
            ...
            if (g_bw_limit_bits < 1.0) {
                // enforce a minimal limit of 1 bit/s if invalid
                g_bw_limit_bits = 1.0;
            }
            i++;
        } else {
