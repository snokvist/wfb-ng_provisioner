...
def map_rssi_to_tx_power(rssi, tx_rssi_min=TX_RSSI_MIN, tx_rssi_max=TX_RSSI_MAX,
    ...
    This mapping is inverted relative to bitrate: a lower RSSI (worse signal)
    yields a higher TX power.
    
    For rssi <= tx_rssi_min, return tx_power_high.
    For rssi >= tx_rssi_max, return tx_power_low.
    Otherwise, linearly interpolate (inverted) between the two.
    """
    if rssi <= tx_rssi_min:
    ...
    # Invert the ratio: when ratio is 0 (rssi == tx_rssi_min), we want high TX power.
